...
fn build_tuple(x: f32, y: f32, z: f32, w: f32) -> Result<Tuple, Box<dyn (::std::error::Error)>> {
	if w != 0.0 && w != 1.0 {
...
fn point(x: f32, y: f32, z: f32) -> Tuple {
	build_tuple(x, y, z, 1.0).unwrap() // should never fail
}

fn vector(x: f32, y: f32, z: f32) -> Tuple {
	build_tuple(x, y, z, 0.0).unwrap() // should never fail
}
...
mod tests {
	...
	fn test_build_checks_arguments() -> () {
		let bad_tup_1 = build_tuple(0.0, 0.0, 0.0, -1.0);
		assert!(bad_tup_1.is_err());
		// assert_eq!(bad_tup_1, Err(CustomError::InvalidArgument));

		let bad_tup_2 = build_tuple(0.0, 0.0, 0.0, 2.0);
		// assert_eq!(bad_tup_2, Err(CustomError::InvalidArgument));
		assert!(bad_tup_2.is_err());

		let bad_tup_3 = build_tuple(std::f32::NAN, 0.0, 0.0, 0.0);
		assert!(bad_tup_3.is_err());
		let bad_tup_4 = build_tuple(0.0, std::f32::NAN, 0.0, 0.0);
		assert!(bad_tup_4.is_err());
		let bad_tup_5 = build_tuple(0.0, 0.0, std::f32::NAN, 0.0);
		assert!(bad_tup_5.is_err());

		let tuple_1 = build_tuple(0.0, 0.0, 0.0, 0.0);
		assert!(tuple_1.is_ok());
		let good_tup_2 = build_tuple(0.0, 0.0, 0.0, 1.0);
	...
	fn test_add_tuples() -> () {
		...
		let sum = (tuple_1 + tuple_2).unwrap();
		assert_abs_diff_eq!(
			sum,
			Tuple {
				x: 2.2,
	...
	fn test_tuple_negation() {
		let tuple = build_tuple(1.0, -2.0, 3.0, 1.0).unwrap();
		let negated = -tuple;
